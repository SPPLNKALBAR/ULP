...
export function LaporanSaifi() {
  const [data, setData] = useState<SaifiData[]>([]);
      ...
      Papa.parse(csvText, {
        complete: (results) => {
          ...
          for (let i = 25; i <= 36; i++) {
            ...
            const bulan = row[2];
            if (!bulan || bulan.trim() === '' || bulan.trim().toUpperCase() === 'BULAN') continue;
  ...
  if (loading && data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-zinc-500 dark:text-zinc-400">Memuat data SAIFI...</p>
      </div>
    );
  }
        ...
        <div className="h-[300px] w-full pt-4">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Line type="monotone" dataKey="target" name="Target" stroke="#a7f3d0" strokeWidth={2} activeDot={{ r: 6 }}>
                <LabelList dataKey="target" position="top" fill="#065f46" fontSize={11} offset={10} formatter={(val: number) => val > 0 ? val.toLocaleString('id-ID') : ''} />
              </Line>
              <Line type="monotone" dataKey="realisasiKumulatif" name="Realisasi Kumulatif" stroke="#10b981" strokeWidth={2} activeDot={{ r: 6 }}>
                <LabelList dataKey="realisasiKumulatif" position="top" fill="#10b981" fontSize={11} offset={10} formatter={(val: number) => val > 0 ? val.toLocaleString('id-ID') : ''} />
              </Line>
            </LineChart>
          </ResponsiveContainer>
        </div>
