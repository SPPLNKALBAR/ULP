    ...
    try {
      ...
      Papa.parse(csvText, {
        complete: (results) => {
          ...
          const parsedData: EnsData[] = [];
          
          for (let i = 41; i <= 52; i++) {
            const row = rows[i];
            if (!row || row.length < 16) continue;
            
            const bulan = row[2]?.trim() || '';
            if (!bulan || bulan.toUpperCase() === 'BULAN') continue;
  ...
  if (loading && data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-zinc-500 dark:text-zinc-400">Memuat data ENS...</p>
      </div>
    );
  }
        ...
        <div className="h-[300px] w-full pt-4">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Line type="monotone" dataKey="target" name="Target" stroke="#fde68a" strokeWidth={2} activeDot={{ r: 6 }}>
                <LabelList dataKey="target" position="top" fill="#854d0e" fontSize={11} offset={10} formatter={(val: number) => val > 0 ? val.toLocaleString('id-ID') : ''} />
              </Line>
              <Line type="monotone" dataKey="realisasiKumulatif" name="Realisasi Kumulatif" stroke="#f59e0b" strokeWidth={2} activeDot={{ r: 6 }}>
                <LabelList dataKey="realisasiKumulatif" position="top" fill="#f59e0b" fontSize={11} offset={10} formatter={(val: number) => val > 0 ? val.toLocaleString('id-ID') : ''} />
              </Line>
            </LineChart>
          </ResponsiveContainer>
        </div>
